...
void TForm1::NewGame()
{
        ...
        for (int i=0; i<2; i++)
        {
                for (int j=0; j<2; j++)
                {
                        ...
                        shapes[i][14+j].imageIndex = 6*i + 5 - j;
                        shapes[i][14+j].isActive = true;
                        shapes[i][14+j].player = i;
                        shapes[i][14+j].shapeId = 5 - j;
                }
        }
...
void TForm1::Draw()
{
        ...
        if (castling && !firstStep[currentPlayer][0])
        {
                ...
                x = shapes[currentPlayer][14].point.x;
                y = shapes[currentPlayer][14].point.y;
                transferLayer->Canvas->FillRect(Rect(x*56, y*56, (x+1)*56, (y+1)*56));
        }
...
void TForm1::MoveShape()
{
        ...
        {
                if (castling && cursor.x == shapes[currentPlayer][14].point.x)
                {
                        shapes[currentPlayer][14].point.x = 2 + 2*currentShape->point.x/7;
...
void TForm1::CheckShah()
{
        ...
        for (int j=0; j<16; j++)
        {
                *currentShape = shapes[currentPlayer][j];
                ...
                GetPossibleSteps();
                //king
                x = shapes[(currentPlayer+1)%2][14].point.x;
                y = shapes[(currentPlayer+1)%2][14].point.y;
...
void TForm1::CheckSteps()
{
        ...
        *savedCurrentShapeValue = *currentShape;
        ...
        if (castling)
        {
                ...
                copyX = shapes[currentPlayer][14].point.x;
                shapes[currentPlayer][14].point.x = 3 - (currentPlayer + 1)%2 + 2*currentShape->point.x/7;
                currentShape->point.x = copyX;
                currentShape = emptyAddress;
                CheckShah();
                shapes[currentPlayer][14].point.x = copyX;
